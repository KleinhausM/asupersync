...
impl LabRuntime {
    ...
    pub fn check_invariants(&self) -> Vec<InvariantViolation> {
        ...
        if self.is_quiescent() && self.state.pending_obligation_count() > 0 {
            violations.push(InvariantViolation::ObligationLeak {
                count: self.state.pending_obligation_count(),
            });
        }
...
impl std::fmt::Display for InvariantViolation {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::ObligationLeak { count } => write!(f, "{count} obligations leaked"),
...
mod tests {
    ...
    use crate::types::{Budget, ObligationId, TaskId};
